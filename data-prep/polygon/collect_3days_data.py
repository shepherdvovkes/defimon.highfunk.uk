#!/usr/bin/env python3
"""
Collect Polygon network data for the last 3 days and store in PostgreSQL
"""
...
class Polygon3DaysDataCollector:
    def __init__(self):
        ...
        # Collection settings
        self.batch_size = 200  # blocks per batch (увеличили для 3 дней)
        self.max_concurrent_requests = 15  # больше параллельных запросов
        self.rate_limit_delay = 0.05  # меньше задержка
    ...
    async def process_block_batch(self, block_numbers):
        ...
        async def process_block(block_number):
            async with semaphore:
                ...
                # Process transactions (все транзакции в блоке)
                transactions = block_data.get('transactions', [])
                tx_count = 0
                receipt_count = 0
                
                for tx in transactions:  # Убрали лимит - собираем все транзакции
                    # Get receipt
    ...
    async def collect_3days_data(self):
        """Collect data for the last 3 days"""
        print("🚀 Starting Polygon 3 Days Data Collection")
        print("=" * 60)
        ...
        try:
            ...
            three_days_ago_block = self.calculate_3days_ago_block(latest_block)
            total_blocks = latest_block - three_days_ago_block
            
            print(f"\n📊 Collection Plan:")
            print(f"  Latest block: {latest_block:,}")
            print(f"  3 days ago block: {three_days_ago_block:,}")
            ...
            # Confirm collection
            response = input("\n🤔 Proceed with 3 days collection? (y/N): ")
            if response.lower() != 'y':
...
async def main():
    """Main function"""
    collector = Polygon3DaysDataCollector()
    await collector.collect_3days_data()
