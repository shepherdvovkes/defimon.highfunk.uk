...
# =============================================================================
# POLKADOT NETWORK DEPLOYMENT ON CTHULHU.LOCAL - FULL CHAIN ARCHIVE MODE
# Deploys Polkadot network on local host cthulhu.local with complete blockchain archive
# =============================================================================
...
POLKADOT_BASE_PATH="/var/lib/polkadot"
...
# Check prerequisites
check_prerequisites() {
    print_header "Checking Prerequisites for Polkadot Cthulhu Local Deployment"
...
test_ssh_connectivity() {
    ...
    if ssh -i "$CTHULHU_SSH_KEY" -o ConnectTimeout=10 -o BatchMode=yes -o PasswordAuthentication=no "$CTHULHU_USER@$CTHULHU_HOST" "echo 'SSH connection successful'" 2>/dev/null; then
        ...
        print_status "5. SSH key is in ~/.ssh/cthulhu"
        exit 1
    fi
}
...
create_directories() {
    ...
    ssh -i "$CTHULHU_SSH_KEY" -o PasswordAuthentication=no "$CTHULHU_USER@$CTHULHU_HOST" << EOF
        # Create directories
        sudo mkdir -p $POLKADOT_BASE_PATH
        sudo mkdir -p /var/log/polkadot
        sudo mkdir -p /etc/polkadot
        sudo mkdir -p /etc/prometheus
        
        # Set permissions
        sudo chown -R $CTHULHU_USER:$CTHULHU_USER $POLKADOT_BASE_PATH
        sudo chown -R $CTHULHU_USER:$CTHULHU_USER /var/log/polkadot
        sudo chown -R $CTHULHU_USER:$CTHULHU_USER /etc/polkadot
...
services:
  polkadot-cthulhu-archive:
    ...
    volumes:
      ...
      - /var/log/polkadot:/var/log/polkadot
    command: >
...
verify_deployment() {
    print_header "Verifying Polkadot Cthulhu Local Deployment"
...
create_management_script() {
    ...
    cat > "$PROJECT_ROOT/scripts/manage-polkadot-cthulhu-local.sh" << 'EOF'
#!/bin/bash
...
EOF
    ...
    chmod +x "$PROJECT_ROOT/scripts/manage-polkadot-cthulhu-local.sh"
    print_success "Management script created: scripts/manage-polkadot-cthulhu-local.sh"
}
...
generate_summary() {
    ...
    cat > "$PROJECT_ROOT/POLKADOT_CTHULHU_LOCAL_DEPLOYMENT_SUMMARY.md" << EOF
...
EOF
    ...
    print_success "Deployment summary generated: POLKADOT_CTHULHU_LOCAL_DEPLOYMENT_SUMMARY.md"
}

# Main deployment function
main() {
    print_cthulhu_header "POLKADOT CTHULHU LOCAL ARCHIVE DEPLOYMENT"
    print_status "Starting Polkadot network deployment on cthulhu.local with full chain archive mode"
    ...
    print_success "Polkadot Cthulhu local archive node has been successfully deployed!"
    print_status "Use './scripts/manage-polkadot-cthulhu-local.sh' to manage the deployment"
    print_status "Check 'POLKADOT_CTHULHU_LOCAL_DEPLOYMENT_SUMMARY.md' for detailed information"
}
