...
case "$1" in
    status)
        ...
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "export PATH=/usr/local/bin:\$PATH && /usr/local/bin/docker ps --format 'table {{.Names}}\t{{.Status}}'"
        ;;
    
    start)
        echo "Starting Polkadot Cthulhu services..."
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "export PATH=/usr/local/bin:\$PATH && cd /Users/vovkes && /usr/local/bin/docker-compose up -d"
        ;;
    
    stop)
        echo "Stopping Polkadot Cthulhu services..."
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "export PATH=/usr/local/bin:\$PATH && cd /Users/vovkes && /usr/local/bin/docker-compose down"
        ;;
    
    restart)
        echo "Restarting Polkadot Cthulhu services..."
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "export PATH=/usr/local/bin:\$PATH && cd /Users/vovkes && /usr/local/bin/docker-compose restart"
        ;;
    
    logs)
        echo "=== Polkadot Cthulhu macOS Logs ==="
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "export PATH=/usr/local/bin:\$PATH && /usr/local/bin/docker logs polkadot-cthulhu-archive --tail 50"
        ;;
    ...
    resources)
        echo "=== Resource Usage ==="
        ssh -i "$CTHULHU_SSH_KEY" "$CTHULHU_USER@$CTHULHU_HOST" "df -h && echo '---' && export PATH=/usr/local/bin:\$PATH && /usr/local/bin/docker stats --no-stream"
        ;;
