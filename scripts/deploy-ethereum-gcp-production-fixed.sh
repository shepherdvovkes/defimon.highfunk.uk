...
# DEFIMON Ethereum Nodes GCP Production Deployment Script (FIXED VERSION)
...
create_production_vm() {
    ...
    gcloud compute instances create ethereum-production \
        ...
        --metadata=startup-script='#! /bin/bash
            # Update system
            apt-get update
            apt-get install -y docker.io docker-compose nginx certbot python3-certbot-nginx curl wget
