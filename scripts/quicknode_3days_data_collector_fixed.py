#!/usr/bin/env python3
"""
QuickNode 3-Day Data Collection Script (Fixed Version)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('quicknode_3days_collection_fixed.log'),
...
class QuickNode3DaysCollectorFixed:
    """QuickNode 3-day data collector with fixed database connection pooling"""
    ...
    async def connect_database(self) -> bool:
        ...
        try:
            ...
            self.db_pool = await asyncpg.create_pool(
                **self.db_config,
                min_size=1,
                max_size=10,  # Limit pool size to avoid overwhelming the database
                command_timeout=60
            )
            logger.info("✅ Connected to PostgreSQL successfully with connection pool")
            return True
        except Exception as e:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                for network_key, network in self.networks.items():
                    ...
                    # Create indexes
                    await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_blocks_timestamp ON {schema_name}.blocks(timestamp)")
                    await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_transactions_block ON {schema_name}.transactions(block_number)")
                    await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_receipts_block ON {schema_name}.receipts(block_number)")
    ...
    async def import_block_to_db(self, network_key: str, block_data: Dict) -> bool:
        """Import block data to database using connection pool"""
        try:
            async with self.db_pool.acquire() as conn:
                schema_name = f"{network_key}_data"
    ...
    async def import_transaction_to_db(self, network_key: str, tx_data: Dict, receipt_data: Optional[Dict] = None) -> bool:
        """Import transaction data to database using connection pool"""
        try:
            async with self.db_pool.acquire() as conn:
                schema_name = f"{network_key}_data"
    ...
    async def process_block_batch(self, session: aiohttp.ClientSession, network: NetworkConfig, block_numbers: List[int]) -> tuple:
        """Process a batch of blocks for a network with proper database connection handling"""
        url = self._get_network_url(network.network_key)
        tasks = []
        ...
        async def process_block(block_number: int) -> tuple:
            async with semaphore:
                try:
                    ...
                    # Process transactions sequentially to avoid database conflicts
        ...
        for block_number in block_numbers:
            txs, receipts = await process_block(block_number)
            total_txs += txs
    ...
    async def collect_all_networks_data(self):
        ...
        logger.info("🚀 Starting QuickNode 3-Day Data Collection (Fixed Version)")
        logger.info("=" * 80)
    ...
    def _print_final_statistics(self):
        ...
        stats_file = f"quicknode_3days_stats_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='QuickNode 3-Day Data Collector (Fixed)')
    ...
    # Create collector
    collector = QuickNode3DaysCollectorFixed(networks=args.networks)
