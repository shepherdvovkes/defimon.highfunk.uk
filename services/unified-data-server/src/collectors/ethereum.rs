...
impl EthereumCollector {
    pub fn new(db: Arc<DatabaseManager>, node_url: String) -> Self {
        Self {
            db,
            node_url,
            client: Client::new(),
    ...
    pub async fn start_collection(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        info!("Starting Ethereum data collection...");
        ...
        loop {
            ...
            if let Err(e) = self.collect_latest_data().await {
                error!("Ethereum collection error: {}", e);
            }
        }
    }

    async fn collect_latest_data(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Collect new blocks
        for block_number in (last_processed + 1)..=latest_block_number {
            if let Err(e) = self.collect_block(block_number).await {
                warn!("Failed to collect block {}: {}", block_number, e);
                continue;
            }
        }
        ...
        info!("Ethereum collection completed for blocks {} to {}", last_processed + 1, latest_block_number);
        Ok(())
    }

    async fn collect_block(&self, block_number: i64) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        // Get block data
        let block_data = self.get_block_by_number(block_number).await?;
        
        // Parse block
        let block = self.parse_block(&block_data).await?;
    ...
    async fn get_latest_block_number(&self) -> Result<i64, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_block_by_number(&self, block_number: i64) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        let response = self.client
            .post(&self.node_url)
            .json(&serde_json::json!({
                "jsonrpc": "2.0",
    ...
    async fn parse_block(&self, block_data: &Value) -> Result<Block, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn parse_transaction(&self, tx_data: &Value, block: &Block) -> Result<Transaction, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_last_processed_block(&self) -> Result<i64, Box<dyn std::error::Error + Send + Sync>> {
        // Get the highest block number from our database
        let blocks = self.db.get_blocks("ethereum", 1, 0).await?;
    ...
    async fn update_network_stats(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        // Get latest block
        let blocks = self.db.get_blocks("ethereum", 1, 0).await?;
        if blocks.is_empty() {
            return Ok(());
